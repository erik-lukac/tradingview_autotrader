...
Fetches the latest orders and positions from Coinbase Advanced Trading,
displaying them in a custom tabular format. Demonstrates various Python best
practices and language features (PEP 484, 557, 435, etc.).
"""
...
async def main_async(num_records: int) -> None:
    ...
    :param num_records: Number of transactions to show (default: 3).
    """
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Show the latest orders and positions from Coinbase Advanced Trading."
    )
    # Positional argument for number of records
    parser.add_argument(
        "number",
        type=int,
        nargs='?',
        default=3,
        help="Number of transactions to show (default: 3).",
    )
